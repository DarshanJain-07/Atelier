...
def test_cascade_power_law():
    ...
    if len(active_cascades) < 10:
        print("Not enough active cascades to plot. The network is highly resistant to virality.")
        return
    ...
    plt.plot(log_x, intercept + slope * log_x, color='red', linestyle='--', 
    ...
    if slope < -0.5 and r_value**2 > 0.5:
        print("-> SUCCESS: The simulation exhibits power-law properties typical of real human social networks!")
    else:
