...
def test_endogenous_events():
    ...
    assert state_stable.get('action_name') is None, "Failed: Action triggered in stable society."
    ...
    assert state_polarized.get('action_name') in ["Civil Protest", "Populist Uprising"], "Failed: No protest triggered."
    ...
    state_divergent = phys_engine.aggregate_society(emotions_divergent, unequal_influence)
    ...
    assert state_divergent.get('action_name') in ["Elite Policy Shift", "Populist Uprising"], "Failed: No divergence event triggered."
