...
def test_influence_susceptibility_ratio():
    ...
    for i, idx in enumerate(seed_indices):
        ...
        engaged_mask = (
            engagement_scores * authority_bonus > adaptive_threshold
        ).float() * sees_post_mask
    ...
    print("Definition:")
    print(
        " - Out-degree: Number of agents highly engaged (>0.5) by an agent's thought."
    )
    print(
        f" - In-degree: Number of times an agent was highly engaged by the {S} shared thoughts."
    )
    print(
        " - Ratio: Out-degree / In-degree (Higher = More influential & less susceptible)\n"
    )

    print(f"Average Out-degree: {out_degree.mean():.2f} (± {out_degree.std():.2f})")
    print(f"  Min: {out_degree.min()}, Max: {out_degree.max()}")
    print(
        f"  25th: {np.percentile(out_degree, 25)}, 50th: {np.percentile(out_degree, 50)}, 75th: {np.percentile(out_degree, 75)}, 95th: {np.percentile(out_degree, 95)}"
    )
    ...
    print(
        f"  25th: {np.percentile(seed_in_degree, 25)}, 50th: {np.percentile(seed_in_degree, 50)}, 75th: {np.percentile(seed_in_degree, 75)}, 95th: {np.percentile(seed_in_degree, 95)}"
    )

    print(f"\nAverage Ratio:      {ratio.mean():.3f} (± {ratio.std():.3f})")
    print(f"  Min: {ratio.min():.3f}, Max: {ratio.max():.3f}")
    print(
        f"  25th: {np.percentile(ratio, 25):.3f}, 50th: {np.percentile(ratio, 50):.3f}, 75th: {np.percentile(ratio, 75):.3f}, 95th: {np.percentile(ratio, 95):.3f}"
    )
    ...
    try:
        ...
        plot_path = os.path.join(
            os.path.dirname(__file__), "influence_susceptibility.png"
        )
    ...
    print(
        f"Correlation (Out-degree vs In-degree): {corr_out_in:.3f} (p={p_out_in:.4f})"
    )
    if corr_out_in < -0.3 and p_out_in < 0.05:
        print(
            "  -> STRONG NEGATIVE: Highly influential agents are rarely swayed themselves (Echo Chamber Leaders)."
        )
    elif corr_out_in > 0.3 and p_out_in < 0.05:
        print(
            "  -> STRONG POSITIVE: The most influential agents are also the most susceptible (Volatile Connectors)."
        )
    else:
        print(
            "  -> NO STRONG LINK: Influence and susceptibility are largely independent traits in this society."
        )
    ...
    if corr_struct > 0.3 and p_struct < 0.05:
        print(
            "  -> ALIGNED: Agents assigned high structural influence naturally command a high Influence/Susceptibility ratio."
        )
    else:
        print(
            "  -> DIVERGENT: Structural influence (wealth/reach) does not guarantee high organic thought leadership."
        )
