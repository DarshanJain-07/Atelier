...
def run_experiment(name, use_hybrid_start=True, generations=300):
    ...
    influence_scores = np.random.lognormal(mean=1.0, sigma=0.5 + config.mutation_temperature, size=config.num_agents)
    ...
    if use_hybrid_start:
        wealth = generate_hybrid_wealth(np.ones(config.num_agents), influence_scores, config.mutation_temperature, config.seed)
    else:
    ...
    top_5_share = initial_wealth[initial_wealth >= top_5_percentile].sum() / initial_wealth.sum()
    
    print(f"Gen 0   | Gini: {initial_gini:.3f} | Top 5% hold {top_5_share*100:.1f}% of wealth")
    ...
    for gen in range(1, generations + 1):
        ...
        if gen in [10, 50, 100, 300]:
            ...
            top_5_share = current_wealth[current_wealth >= top_5_percentile].sum() / current_wealth.sum()
            print(f"Gen {gen:<3} | Gini: {current_gini:.3f} | Top 5% hold {top_5_share*100:.1f}% of wealth")
            
    return current_gini

if __name__ == "__main__":
    run_experiment("Current Hybrid Power-Law Start", use_hybrid_start=True, generations=300)
